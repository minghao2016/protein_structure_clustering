...
sc = SpectralClustering(n_clusters=5, affinity='precomputed', assign_labels="kmeans", random_state=100, n_jobs=-1).fit(matrix)
metrics = ce.clusterEvaluationNoLabels(matrix, sc.labels_)
print(metrics)

sc = SpectralClustering(n_clusters=5, affinity='precomputed', assign_labels="discretize", random_state=100, n_jobs=-1).fit(matrix)
metrics = ce.clusterEvaluationNoLabels(matrix, sc.labels_)
print(metrics)
...
sc = SpectralClustering(n_clusters=5, affinity='nearest_neighbors', assign_labels="discretize", random_state=100, n_jobs=-1).fit(matrix)
metrics = ce.clusterEvaluationNoLabels(matrix, sc.labels_)
print(metrics)

sc = SpectralClustering(n_clusters=5, affinity='rbf', assign_labels="discretize", random_state=100, n_jobs=-1).fit(matrix)
metrics = ce.clusterEvaluationNoLabels(matrix, sc.labels_)
print(metrics)
